...
async def send_request(session, request_id):
    ...
    start = time.time()
    try:
        async with session.post(TARGET_URL, json=TEST_DATA) as response:
            ...
            # 打印前几个请求的响应信息用于调试
            if request_id < 5:
                print(f"Request {request_id}: Status={response.status}, Body={body[:100]}")
            
            if 200 <= response.status < 300:  # 接受所有 2xx 状态码
        ...
        # 打印前几个请求的异常信息
        if request_id < 5:
            print(f"Request {request_id}: Error={str(e)[:100]}")

async def run_load_test():
    """运行负载测试 - HTTP/2 多路复用优化"""
    print(f"开始压力测试...")
    print(f"目标URL: {TARGET_URL}")
    print(f"测试时长: {TEST_DURATION} 秒")
    print(f"并发连接数: {CONCURRENT_CONNECTIONS}")
    print(f"目标请求数: {TOTAL_REQUESTS:,}")
    print(f"协议: HTTP/2 多路复用")
    print("-" * 50)

    stats.start_time = time.time()
    end_time = stats.start_time + TEST_DURATION

    # HTTP/2 连接器配置 - 启用连接复用和多路复用
    connector = aiohttp.TCPConnector(
        limit=CONCURRENT_CONNECTIONS,  # 总连接数限制
        limit_per_host=CONCURRENT_CONNECTIONS,  # 每个主机的连接数
        enable_cleanup_closed=True,  # 启用关闭连接清理
        force_close=False,  # 不强制关闭连接，启用 Keep-Alive
        use_dns_cache=True,  # 启用 DNS 缓存
        ttl_dns_cache=300,  # DNS 缓存 5 分钟
        keepalive_timeout=75,  # Keep-Alive 超时 75 秒
    )
